...
import { explosionPlaces, parsePosition } from './utils';
interface IProps {
...
const isShooting = (lastShoot: number) => (new Date()).getTime() - lastShoot <= 150;
...
PARTICLE_MAP_SOURCE.map((_, i) => getPosOfIndex(i, [800, 600]))
...
const is2sOrLess = (explosion: Explosion) => {
  const time = (explosionPlaces[explosion.grenadeId.replace("_low", "").replace("_high","")]?.time || 0);
  return (Date.now() - time) <= 200;
}


const Grenade = ({ reverseZoom, type, state, visible, position, flames, side, explosions }: { explosions: Explosion[], reverseZoom: string, side: Side | null, flames: boolean, type: RadarGrenadeObject["type"], state: RadarGrenadeObject["state"], visible: boolean, position: number[] }) => {
  if (flames) {
    return null;
  }
  if(type === "smoke" && (state === "landed" || state === "exploded")){
    const accessibleExplosions = explosions.filter(ex => is2sOrLess(ex) && getDistance(position, ex.position) <= MAX_DISTANCE_BETWEEN_FRAG_AND_SMOKE);
    //console.log(explosionPlaces);
    return (
      <div className={`grenade ${type} ${state} ${side || ''} ${visible ? 'visible':'hidden'}`}
        ...
        <div className="content" style={{ width: config.smokeSize, height: config.smokeSize }}>
          ...
          <div className="background">
            {PARTICLE_MAP_SOURCE.map((_, i) => <Particle key={`${i}`} origin={position} index={i} explosions={accessibleExplosions}/>)}
          </div>
      </div>
      </div>
    )
    ...
    <div
      ...
      style={{
        ...
        <div className="content" style={{
          ...
          <div className="label">{player.isAlive ? player.label : 'X'}</div>
        </div>
    </div>
  )
}
